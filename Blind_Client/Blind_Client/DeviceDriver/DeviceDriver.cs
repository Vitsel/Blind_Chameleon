    ...
    public class HH_Lib
    {
        ...
        public List<DEVICE_INFO> GetAll()
        {
            ...
            try
            {
                ...
                StringBuilder DeviceServiceName = new StringBuilder("");
                ...
                for (i = 0; Native.SetupDiEnumDeviceInfo(hDevInfo, i, DeviceInfoData); i++)
                {
                    ...
                    Native.SetupDiGetDeviceRegistryProperty(hDevInfo, DeviceInfoData, Native.SPDRP_SERVICE, 0, DeviceServiceName, Native.MAX_DEV_LEN, IntPtr.Zero);
                    ...
                    HWList.Add(new DEVICE_INFO { name = DeviceName.ToString(), friendlyName = DeviceFriendlyName.ToString(), hardwareId = DeviceHardwareId.ToString(), status = deviceStatus, statusstr = dstatustr,service = DeviceServiceName.ToString() });
                }
        ...
        public bool SetDeviceState(DEVICE_INFO deviceToChangeState, bool bEnable)
        {
            ...
            for (i = 0; Native.SetupDiEnumDeviceInfo(hDevInfo, i, DeviceInfoData); i++)
            {
                ...
                if (DeviceHardwareId.ToString().ToLower().Contains(deviceToChangeState.hardwareId.ToLower()) && DeviceFriendlyName.ToString().ToLower().Contains(deviceToChangeState.friendlyName.ToLower()))
                {
                    ...
                    if (!couldChangeState)
                        throw new Exception("make sure you have administrator privileges");
                    break;
                }
            }
